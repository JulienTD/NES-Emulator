...
impl CPU {
    pub(crate) fn handleBEQ(& mut self, value: u8) -> u8 {
...
mod tests {
    ...
    fn test_beq_branch_taken() {
        ...
        let cycles = cpu.handleBEQ(0x10); // Branch forward by 16
    ...
    fn test_beq_branch_not_taken() {
        ...
        let cycles = cpu.handleBEQ(0x10); // Attempt to branch forward by 16
    ...
    fn test_beq_page_crossing() {
        ...
        cpu.set_status_flag(StatusFlag::Zero, true); // Set Zero flag
        let cycles = cpu.handleBEQ(0x20); // Branch forward by 32 (crosses page)
