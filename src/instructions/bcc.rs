...
impl CPU {
    pub(crate) fn handleBCC(& mut self, value: u8) -> u8 {
...
mod tests {
    ...
    fn test_bcc_branch_taken() {
        ...
        let cycles = cpu.handleBCC(0x10); // Branch forward by 16
    ...
    fn test_bcc_branch_not_taken() {
        ...
        let cycles = cpu.handleBCC(0x10); // Attempt to branch forward by 16
    ...
    fn test_bcc_page_crossing() {
        ...
        cpu.set_status_flag(StatusFlag::Carry, false); // Clear Carry flag
        let cycles = cpu.handleBCC(0x20); // Branch forward by 32 (crosses page)
