...
impl CPU {
    pub(crate) fn handleBPL(& mut self, opt_value: Option<u8>, _opt_address: Option<u16>) -> u8 {
...
mod tests {
    ...
    fn test_bpl_branch_taken() {
        ...
        cpu.set_status_flag(StatusFlag::Negative, false); // Clear Negative flag => positive
        let cycles = cpu.handleBPL(Some(0x10), None); // Branch forward by 16
    ...
    fn test_bpl_branch_not_taken() {
        let mut cpu = new_cpu(Bus::new(Rom::test_rom()));
        cpu.program_counter = 0x1000;
        cpu.set_status_flag(StatusFlag::Negative, true); // Set Negative flag => not positive
        let cycles = cpu.handleBPL(Some(0x10), None); // Attempt to branch forward by 16
    ...
    fn test_bpl_page_crossing() {
        ...
        cpu.set_status_flag(StatusFlag::Negative, false); // Clear Negative flag => branch
        let cycles = cpu.handleBPL(Some(0x20), None); // Branch forward by 32 (crosses page)
