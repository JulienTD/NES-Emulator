...
impl CPU {
    pub(crate) fn handleBMI(& mut self, value: u8) -> u8 {
...
mod tests {
    ...
    fn test_bmi_branch_taken() {
        ...
        let cycles = cpu.handleBMI(0x10); // Branch forward by 16
    ...
    fn test_bmi_branch_not_taken() {
        ...
        let cycles = cpu.handleBMI(0x10); // Attempt to branch forward by 16
    ...
    fn test_bmi_page_crossing() {
        ...
        cpu.set_status_flag(StatusFlag::Negative, true); // Set Negative flag
        let cycles = cpu.handleBMI(0x20); // Branch forward by 32 (crosses page)
